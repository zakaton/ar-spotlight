...
AFRAME.registerComponent("crawling-cursor", {
  ...
  init: function () {
    ...
    this.positionCursor = function (intersection) {
      if (intersection !== null) {
        ...
        var lookAtTarget = new THREE.Vector3().addVectors(
          intersection.point,
          global_normal
        );
        ...
        var cursorPosition = new THREE.Vector3().addVectors(
          intersection.point,
          global_normal.multiplyScalar(this.data.offset)
        );
  ...
  tick: function (time, delta) {
    ...
    if (
      this.data.enabled &&
      this.intersecting > 0 &&
      time - this.lastUpdate > this.data.interval
    ) {
      this.lastUpdate = time;
      this.positionCursor(
        this.el.components.raycaster.getIntersection(
          this.el.components.raycaster.intersectedEls.find(
            (el) =>
              this.data.target !== el && el.classList.contains("allow-ray")
          )
        )
      );
    }
